<?php
  ...
  if (!isset($postdate['title']) || $postdate['del_flg'] === "1") {
    header('Location: index.php');
    exit();
  }
  ...
  while($likeDate = $stm->fetch()) {
    if ($likeDate['name'] === $_SESSION['login']['name']) {
      ...
      $stmt->bindValue(':favo', 1, PDO::PARAM_INT);
  ...
  $stmt = $db->prepare('INSERT INTO likes SET favo = :favo, name = :name, postcode = :postcode, created_at = now()');
  $stmt->bindValue(':favo', 1, PDO::PARAM_INT);
  $stmt->bindValue(':name', $_SESSION['login']['name'], PDO::PARAM_STR);
  $stmt->bindValue(':postcode', $postdate['id'], PDO::PARAM_INT);
  $stmt->execute();
